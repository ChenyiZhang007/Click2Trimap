...
class InteractiveDemoApp(ttk.Frame):
    def __init__(self, master, args, c2t_model, matting_model):
        ...
        self.master = master
        master.title("Click2Trimap Demo")
        master.withdraw()
        ...
        self._init_state()
        self._add_menu()
        self._add_canvas_img()
        self._add_canvas_alpha()
        self._add_buttons()
    ...
    def _add_menu(self):
        ...
        self.save_mask_btn = FocusButton(self.menubar, text='Save trimap', command=self._save_mask_callback)
        self.save_mask_btn.pack(side=tk.LEFT)
        self.save_mask_btn.configure(state=tk.DISABLED)

        self.save_alpha_btn = FocusButton(self.menubar, text='Save alpha', command=self._save_alpha_callback)
        self.save_alpha_btn.pack(side=tk.LEFT)
        self.save_alpha_btn.configure(state=tk.DISABLED)
    ...
    def _add_canvas_img(self):
        self.canvas_frame = FocusLabelFrame(self, text="Image")
        self.canvas_frame.rowconfigure(0, weight=1)
        self.canvas_frame.columnconfigure(0, weight=1)

        self.canvas = tk.Canvas(self.canvas_frame, highlightthickness=0, cursor="hand1", width=800, height=400)
    ...
    def _add_canvas_alpha(self):
        self.canvas_frame_alpha = FocusLabelFrame(self, text="Alpha")
        self.canvas_frame_alpha.rowconfigure(0, weight=1)
        self.canvas_frame_alpha.columnconfigure(0, weight=1)

        self.canvas_alpha = tk.Canvas(self.canvas_frame_alpha, highlightthickness=0, cursor="hand1", width=800, height=400)
        self.canvas_alpha.grid(row=0, column=0, sticky='nswe', padx=5, pady=5)

        self.alpha_on_canvas = None
        self.canvas_frame_alpha.pack(side=tk.LEFT, fill="both", expand=True, padx=5, pady=5)
    ...
    def _add_buttons(self):
        ...
        ################################### Background ###################################
        self.background_frame = FocusLabelFrame(master, text="Change Background")
        self.background_frame.pack(side=tk.TOP, fill=tk.X, padx=10, pady=3)

        self.change_background_button_1 = \
            FocusButton(self.background_frame, text=' 1', bg='#ffe599', fg='black', width=10, height=2,
                        state=tk.NORMAL, command=self.controller.change_background_1)
        self.change_background_button_1.pack(side=tk.LEFT, fill=tk.X, padx=10, pady=3)

        self.change_background_button_2 = \
            FocusButton(self.background_frame, text=' 2', bg='#ffe599', fg='black', width=10, height=2,
                        state=tk.NORMAL, command=self.controller.change_background_2)
        self.change_background_button_2.pack(side=tk.LEFT, fill=tk.X, padx=10, pady=3)

        self.change_background_button_3 = \
            FocusButton(self.background_frame, text=' 3', bg='#ffe599', fg='black', width=10, height=2,
                        state=tk.NORMAL, command=self.controller.change_background_3)
        self.change_background_button_3.pack(side=tk.LEFT, fill=tk.X, padx=10, pady=3)
        ...
        self.zoomin_options_frame.pack_forget()
        ...
        self.brs_options_frame.pack_forget()
        ...
        self.prob_thresh_frame.pack_forget()
        ...
        # self.background_frame = FocusLabelFrame(master, text="Change Background")
        # self.background_frame.pack(side=tk.TOP, fill=tk.X, padx=10, pady=3)
        # FocusButton(self.background_frame, from_=0.0, to=1.0, command=self.change_background,
        #                      variable=self.state['alpha_blend']).pack(padx=10, anchor=tk.CENTER)
        ...
        self.click_radius_frame.pack_forget()
    ...
    def _load_image_callback(self):
        ...
        if self._check_entry(self):
            ...
            if len(filename) > 0:
                ...
                self.save_alpha_btn.configure(state=tk.NORMAL)
    ...
    def _save_alpha_callback(self):
        self.menubar.focus_set()
        if self._check_entry(self):
            mask = self.controller.result_mask
            alpha = self.controller.alpha_save
            # trimap = self.controller.trimap_save
            ...
            alpha_name = filedialog.asksaveasfilename(parent=self.master, initialfile='alpha.png', filetypes=[
                ("PNG image", "*.png"),
                ("BMP image", "*.bmp"),
                ("All files", "*.*"),
            ], title="Save the current mask as...")
    ...
    def _update_image(self, reset_canvas=False, backgroung_flag=None):
        ...
        if self.alpha_on_canvas is None:
            self.alpha_on_canvas = CanvasImage(self.canvas_frame_alpha, self.canvas_alpha)
            self.alpha_on_canvas.register_click_callback(self._click_callback)

        self._set_click_dependent_widgets_state()
        if image is not None:
            self.image_on_canvas.reload_image(Image.fromarray(image), reset_canvas)
        if alpha is not None:
            # cv2.imwrite('pred.png',alpha)
            # self.image_on_canvas.reload_image(Image.fromarray(alpha[:,:,0]), reset_canvas)
            if backgroung_flag == None:
                frame_new = alpha[:,:,0]

            if backgroung_flag == 1:
                background = cv2.imread('background\\street.jpg')
                background = cv2.cvtColor(background, cv2.COLOR_BGR2RGB)
                background = cv2.resize(background, (image.shape[1], image.shape[0]))
                alpha_normalized = alpha/255
                frame_new = image_source * alpha_normalized.repeat(3,2) + background * (1 - alpha_normalized.repeat(3,2))
                frame_new = frame_new.astype(np.uint8)

            if backgroung_flag == 2:
                background = cv2.imread('background\\space.jpg')
                background = cv2.cvtColor(background, cv2.COLOR_BGR2RGB)
                background = cv2.resize(background, (image.shape[1], image.shape[0]))
                alpha_normalized = alpha/255
                frame_new = image_source * alpha_normalized.repeat(3,2) + background * (1 - alpha_normalized.repeat(3,2))
            ...
            if backgroung_flag == 3:
                background = cv2.imread('background\\beach.jpg')
                background = cv2.cvtColor(background, cv2.COLOR_BGR2RGB)
                background = cv2.resize(background, (image.shape[1], image.shape[0]))
                alpha_normalized = alpha/255
                frame_new = image_source * alpha_normalized.repeat(3,2) + background * (1 - alpha_normalized.repeat(3,2))
            ...
            self.alpha_on_canvas.reload_image(Image.fromarray(frame_new), reset_canvas)
        else:
            black = np.zeros_like(image)
            self.alpha_on_canvas.reload_image(Image.fromarray(black), reset_canvas)
