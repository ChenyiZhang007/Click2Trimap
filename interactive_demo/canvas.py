...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas):
        ...
        self.canvas.bind('<Button-2>', self.__roll_mouse_button)  # remember canvas position 
        self.canvas.bind('<Button-3>', self.__right_mouse_button)  # remember canvas position
    ...
    def __left_mouse_button(self, event):
        ...
        if coords is not None:
            self._click_callback(flag=0, x=coords[0], y=coords[1])

    def __roll_mouse_button(self, event):
        if self._click_callback is None:
            return
        ...
        if coords is not None:
            self._click_callback(flag=1, x=coords[0], y=coords[1])
    ...
    def __right_mouse_button(self, event):
        ...
        if coords is not None:
            self._click_callback(flag=2, x=coords[0], y=coords[1])
