...
def draw_with_blend_and_clicks(img, mask=None, alpha=0.6, clicks_list=None, pos_color=(0, 255, 0),
    ...
    if clicks_list is not None and len(clicks_list) > 0:
        ...
        fg_points = [click.coords for click in clicks_list if click.flag==0]
        uk_points = [click.coords for click in clicks_list if click.flag==1]
        bg_points = [click.coords for click in clicks_list if click.flag==2]


        result = draw_points(result, fg_points, fg_color, radius=radius)
        result = draw_points(result, bg_points, bg_color, radius=radius)
        result = draw_points(result, uk_points, uk_color, radius=radius)
